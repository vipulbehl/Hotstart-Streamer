...
if ($last==$testl){
	...
	$options  = array('http' => array('user_agent' => 'custom user agent string', 'X-Forwarded-For'=> '27.123.127.255'));
	...
	if(isset($argv[3]) AND isset($argv[2])){
		...
		echo shell_exec("$argv[5]livestreamer --http-header \"X-Forwarded-For=27.123.127.255\" \"$m3u8\" \"$argv[3]\" -o \"$argv[4]$filename.ts\" &");
		echo "Done.\n";
		}
		else{ 
		}
	...
	if(isset($argv[2])){
		...
		echo "Starting livestreamer...\n\n";
		echo shell_exec("$argv[4]livestreamer --http-header \"X-Forwarded-For=27.123.127.255\" \"$m3u8\" \"$argv[2]\" -o \"$argv[3]$filename.ts\" &");
		echo "Done.\n";
		}
		else{ 
		echo "Starting livestreamer...\n\n";
		echo shell_exec("$argv[4]livestreamer --http-header \"X-Forwarded-For=27.123.127.255\" \"$m3u8\" \"$argv[2]\"  &");
		echo "Done.\n";
		}
		}
